# Library for Section 4: Results and Discussion
library(ggplot2)
library(dplyr)
library(tidyr)
...
prediction_res <- function(train_X, train_y, test_X, test_y, y) {
  ...
  RMSE <- c()
  for(i in 1:k) {
    fit <- rkriging::Fit.Kriging(train_X, train_y[,i], kernel.parameters = list(type = "Gaussian"))
    pred <- rkriging::Predict.Kriging(fit, test_X)$mean
    mae <- mean(abs(test_y[,i] - pred))
    NMAE <- c(NMAE, (100*mae) / mean(abs(y[,i])))
    RMSE <- c(RMSE, sqrt(mean((test_y[,i] - pred)^2)))
  }
...
simu_each <- function(X, y, n, k = 30, method) {
  all_y1_nmae <- all_y2_nmae <- all_y1_rmse <- all_y2_rmse <- c()
  boundary_y1_nmae <- boundary_y2_nmae <- boundary_y1_rmse <- boundary_y2_rmse <- c()
  interior_y1_nmae <- interior_y2_nmae <- interior_y1_rmse <- interior_y2_rmse <- c()
  rt <- c()
  duplicate <- c()
  ...
  for(i in 1:k) {
    ...
    all_y2_nmae <- c(all_y2_nmae, all_res$NMAE[2])
    all_y1_rmse <- c(all_y1_rmse, all_res$RMSE[1])
    all_y2_rmse <- c(all_y2_rmse, all_res$RMSE[2])
    boundary_y1_nmae <- c(boundary_y1_nmae, boundary_res$NMAE[1])
    boundary_y2_nmae <- c(boundary_y2_nmae, boundary_res$NMAE[2])
    boundary_y1_rmse <- c(boundary_y1_rmse, boundary_res$RMSE[1])
    boundary_y2_rmse <- c(boundary_y2_rmse, boundary_res$RMSE[2])
    interior_y1_nmae <- c(interior_y1_nmae, interior_res$NMAE[1])
    interior_y2_nmae <- c(interior_y2_nmae, interior_res$NMAE[2])
    interior_y1_rmse <- c(interior_y1_rmse, interior_res$RMSE[1])
    interior_y2_rmse <- c(interior_y2_rmse, interior_res$RMSE[2])
  }
  ans <- list(
    all_y1_nmae=all_y1_nmae, all_y1_rmse=all_y1_rmse, all_y2_nmae=all_y2_nmae, all_y2_rmse=all_y2_rmse,
    boundary_y1_nmae=boundary_y1_nmae, boundary_y1_rmse=boundary_y1_rmse, boundary_y2_nmae=boundary_y2_nmae, boundary_y2_rmse=boundary_y2_rmse,
    interior_y1_nmae=interior_y1_nmae, interior_y1_rmse=interior_y1_rmse, interior_y2_nmae=interior_y2_nmae, interior_y2_rmse=interior_y2_rmse,
...
simu <- function(X, y, n, k = 30) {
  ...
  all_y2_nmae_df <- data.frame(Random = rand_res$all_y2_nmae, Sobol = sobol_res$all_y2_nmae, K_medoids = K_medoids_res$all_y2_nmae, SP = sp_res$all_y2_nmae, SSP = ssp_res$all_y2_nmae)
  all_y2_rmse_df <- data.frame(Random = rand_res$all_y2_rmse, Sobol = sobol_res$all_y2_rmse, K_medoids = K_medoids_res$all_y2_rmse, SP = sp_res$all_y2_rmse, SSP = ssp_res$all_y2_rmse)
  ...
  boundary_y2_nmae_df <- data.frame(Random = rand_res$boundary_y2_nmae, Sobol = sobol_res$boundary_y2_nmae, K_medoids = K_medoids_res$boundary_y2_nmae, SP = sp_res$boundary_y2_nmae, SSP = ssp_res$boundary_y2_nmae)
  boundary_y2_rmse_df <- data.frame(Random = rand_res$boundary_y2_rmse, Sobol = sobol_res$boundary_y2_rmse, K_medoids = K_medoids_res$boundary_y2_rmse, SP = sp_res$boundary_y2_rmse, SSP = ssp_res$boundary_y2_rmse)
  ...
  interior_y2_nmae_df <- data.frame(Random = rand_res$interior_y2_nmae, Sobol = sobol_res$interior_y2_nmae, K_medoids = K_medoids_res$interior_y2_nmae, SP = sp_res$interior_y2_nmae, SSP = ssp_res$interior_y2_nmae)
  interior_y2_rmse_df <- data.frame(Random = rand_res$interior_y2_rmse, Sobol = sobol_res$interior_y2_rmse, K_medoids = K_medoids_res$interior_y2_rmse, SP = sp_res$interior_y2_rmse, SSP = ssp_res$interior_y2_rmse)
  
  ans <- list(
    rt_df=rt_df, duplicate_df=duplicate_df, 
    all_y1_nmae_df=all_y1_nmae_df, all_y1_rmse_df=all_y1_rmse_df, all_y2_nmae_df=all_y2_nmae_df, all_y2_rmse_df=all_y2_rmse_df,
    boundary_y1_nmae_df=boundary_y1_nmae_df, boundary_y1_rmse_df=boundary_y1_rmse_df, boundary_y2_nmae_df=boundary_y2_nmae_df, boundary_y2_rmse_df=boundary_y2_rmse_df,
    interior_y1_nmae_df=interior_y1_nmae_df, interior_y1_rmse_df=interior_y1_rmse_df, interior_y2_nmae_df=interior_y2_nmae_df, interior_y2_rmse_df=interior_y2_rmse_df)
  return(ans)
}
