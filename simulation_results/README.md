...
- `Figure_2_results.rds`, `Figure_3_results.rds`, and `Figure_4_results.rds` save their designs and plot inputs. All three use 10,000 Sobol candidates generated by `spacefillr`; Figure 3 evaluates 40,000 Sobol points transformed to the simplex plus its three vertices. `D7_max_level.Rdata` and `.csv` save coordinate minima/maxima and selected indices (indices in Rdata); `D4_D7_runtime.csv` saves median runtimes.
